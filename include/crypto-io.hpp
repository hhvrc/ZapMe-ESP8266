...
public:
  ...
  std::size_t readBytes(std::uint8_t* data, std::size_t length) { return readBytes(reinterpret_cast<char*>(data), length); }
...
public:
  ...
  std::size_t write(const char* data, std::size_t length) { return write(reinterpret_cast<const std::uint8_t*>(data), length); }
  std::size_t write(const char* string) { return write(string, std::strlen(string)); }
  std::size_t write(const String& string) { return write(string.c_str(), string.length()); }
